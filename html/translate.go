...
var uiTranslations = map[string][]TagStr{
	...
	"status-processing": []TagStr{
		TagStr{language.AmericanEnglish, "Payment processing: A payment is on the way, but we're still waiting for the required amount of confirmations on the blockchain."},
		TagStr{language.German, "Zahlung wird verarbeitet: Eine Zahlung wurde angekündigt, aber wir warten noch auf die erforderliche Anzahl Bestätigungen auf der Blockchain."},
	},
